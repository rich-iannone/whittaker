...
for k in [5, 10, 15, 20, 30]:
    cv = wk.cross_validate(f"y ~ s(x, k={k})", data, method="REML", seed=42)
    results[k] = cv
...
# Compare families
cv_gauss = wk.cross_validate("y ~ s(x)", count_data, method="REML", seed=42)
cv_pois = wk.cross_validate(
    "y ~ s(x)", count_data, family=wk.Poisson(), method="REML", seed=42
)
...
cv_gauss_mse = wk.cross_validate(
    "y ~ s(x)", count_data, method="REML", metric="mse", seed=42
)
cv_pois_mse = wk.cross_validate(
    "y ~ s(x)", count_data, family=wk.Poisson(), method="REML", metric="mse", seed=42
)
...
```{python}
result = wk.cross_validate("y ~ s(x)", data, method="REML", seed=42)
...
for k in [5, 10, 20]:
    cv = wk.cross_validate("y ~ s(x)", data, n_folds=k, method="REML", seed=42)
...
cv_no_select = wk.cross_validate(
    "y ~ s(x) + s(noise)", data_noise, method="REML", seed=42
)
cv_select = wk.cross_validate(
    "y ~ s(x) + s(noise)", data_noise, method="REML", select=True, seed=42
)
...
for name, formula in candidates.items():
    cv = wk.cross_validate(formula, data_noise, method="REML", seed=42)
    flag = ""
