...
for label in term_preds.labels:
    ...
    chart = (band + line + zero).properties(
        width=280, height=220, title=label
    )
    charts.append(chart)
...
(pts_pois + line_fit + line_true).properties(
    width=500, height=300, title="Poisson GAM: fitted rate (orange) vs. true rate (dashed)"
)
```
...
print(f"Probability range: [{preds_bin.values.min():.3f}, {preds_bin.values.max():.3f}]")
print(f"Log-odds range: [{preds_bin.linear_predictor.min():.3f}, {preds_bin.linear_predictor.max():.3f}]")
```
...
(pts_bin + line_bin + line_true_bin).properties(
    width=500, height=300, title="Binomial GAM: fitted probability (orange) vs. true (dashed)"
)
```
