...
).properties(width="container", height=300, title="Multi-response GAM predictions")
```
...
(heatmap + labels).properties(width="container", height=250, title="Residual correlation matrix")
```
...
The `summary()` method reports per-response fit statistics, including EDF, deviance, and smoothing
parameters for each response.
...
You can now fit multi-response GAMs with shared or response-specific formulas, estimate residual
correlations, and access per-response models for further inspection.

## Where to go next

- **[Prediction and inference](06-prediction.qmd)**: confidence intervals and term-level
  predictions for individual response models.
- **[Model diagnostics](07-diagnostics.qmd)**: check each response model's residuals and basis
  adequacy.
- **[Functional regression](16-functional.qmd)**: another multi-predictor setting where the
  covariates are entire curves.
