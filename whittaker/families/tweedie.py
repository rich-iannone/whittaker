...
class Tweedie(Family):
    ...
    The Tweedie distribution is an exponential dispersion model with variance function
    V(μ) = μ^p. The variance power *p* determines the distribution shape:

    - `1 < p < 2`: compound Poisson-Gamma — continuous with a point mass at zero. This is the
      most common case (insurance claims, biomass, rainfall).
    - `p = 1`: Poisson (use `Poisson()` instead for exact computations).
    - `p = 2`: Gamma (use `Gamma()` instead for exact computations).
    - `p > 2`: positive continuous (inverse Gaussian at `p = 3`).

    Parameters
    ----------
    p:
        Variance power, must satisfy `1 < p < 2` or `p > 2`.
    """
