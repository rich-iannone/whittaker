...
class TestVINotImplemented:
    def test_deviance_positive(self, poisson_data):
        model = GAM("y ~ s(x)", family=Poisson()).fit(poisson_data, method="VI")

        assert model.deviance > 0
        assert np.isfinite(model.deviance)

    def test_aic_finite(self, poisson_data):
        model = GAM("y ~ s(x)", family=Poisson()).fit(poisson_data, method="VI")

        assert np.isfinite(model.aic)

    def test_bic_finite(self, poisson_data):
        model = GAM("y ~ s(x)", family=Poisson()).fit(poisson_data, method="VI")
    ...
    def test_null_deviance_positive(self, poisson_data):
        model = GAM("y ~ s(x)", family=Poisson()).fit(poisson_data, method="VI")

        assert model.null_deviance > 0
        assert model.null_deviance >= model.deviance

    def test_deviance_explained_range(self, poisson_data):
        model = GAM("y ~ s(x)", family=Poisson()).fit(poisson_data, method="VI")
        de = model.deviance_explained
